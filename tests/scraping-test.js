...
async function scrapeTest() {
  ...
  const repositories = await page.evaluate(() => {
    ...
    repoElements.forEach((element, index) => {
      ...
      try {
        ...
        const repoName = repoLink ? repoLink.textContent.trim().replace(/\s+/g, '') : '';
        const repoUrl = repoLink ? 'https://github.com' + repoLink.getAttribute('href') : '';
        
        // 获取描述
        const description = element.querySelector('p');
        const repoDescription = description ? description.textContent.trim() : '';
        ...
        // 获取今日星标数
        const todayStarsElement = element.querySelector('span.d-inline-block.float-sm-right');
        const todayStars = todayStarsElement ? todayStarsElement.textContent.trim() : '0';
  ...
  await fs.writeFile('output/github-trending.json', JSON.stringify(dataToSave, null, 2), 'utf8');
  ...
  repositories.slice(0, 5).forEach((repo, index) => {
    ...
    console.log(`   星标: ${repo.stars} | Fork: ${repo.forks} | 今日: ${repo.todayStars}`);
